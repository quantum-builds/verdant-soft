...
export default function TechnologiesSection({
  ...
  return (
    <section
      id="technologies"
      className="scroll-mt-36 h-[580px] md:h-[630px] lg:h-[590px] xl:h-[705px] 2xl:h-[690px]  4xl:h-[710px] 5xl:h-[750px] w-11/12 xl:w-10/12 mx-auto flex flex-col gap-10 xl:gap-20 mb-20 overflow-hidden text-black"
    >
      <p
      ...
      <div className="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4  gap-5">
        {useCases.map((useCase, index) => (
          <div key={index} className="flex flex-col gap-5 xl:gap-10 ">
            <p className="text-green-gradient text-2xl lg:text-[28px] xl:text-[32px] font-semibold">
              {useCase.title}
            </p>
            <div className="flex flex-col gap-3 lg:gap-5 xl:gap-7">
              {useCase.technologies.map((technology, index) => (
                <div key={index} className="flex gap-3 items-center">
                  ...
                  />
                  <p className="text-lg lg:text-2xl xl:text-[28px]">
                    {technology.title}
                  </p>
                </div>
              ))}
